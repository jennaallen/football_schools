...
```{r raw_school_data, message = FALSE, cache = TRUE, dependson = "years"}
...
```{r show_raw_school_data, echo = FALSE}
 knitr::kable(
   school_data_staging[2:6, ]
 )
```
...
```{r clean_school_data, message = FALSE}
...
```{r clean_football_data, message = FALSE}
...
```{r show_fuzzy_matches, echo = FALSE}
knitr::kable(
  fuzzy_text_res[1:5, ]
)
```
...
```{r add_fuzzy_final_1, eval = FALSE}
...
```{r add_fuzzy_final_2, eval = FALSE}
#this is an iterative process where you can add the correct matches to a final match data table until all schools are matched
...
```{r final_fuzzy_matches, echo = FALSE, message = FALSE}
...
```{r write_dimSchoolYear, eval = FALSE}
...
```{sql sql_view, eval = FALSE, connection = con}
