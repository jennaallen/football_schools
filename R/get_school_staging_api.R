...
years <- seq(2011, 2015) #want data for school years 2011-2015 

# get data for desired years and combine it all in one data table
school_data_staging_api <- rbindlist(lapply(years, function(x) {
  sc_init() %>%
    sc_filter(control == 1:2, main == 1) %>% #filter for public and prive not for profit schools and main campuses
