...
school_data_transform$region <- plyr::mapvalues(school_data_transform$region, 
                                                from = regions$Value, to = regions$ValueLabel)
school_data_transform$control <- plyr::mapvalues(school_data_transform$control, 
                                                 from = control$Value, to = control$ValueLabel)
school_data_transform$iclevel <- plyr::mapvalues(school_data_transform$iclevel, 
                                                 from = level$Value, to = level$ValueLabel)
school_data_transform <- left_join(school_data_transform, states[,2:3], 
                                   by = c("stabbr" = "Value"))

#clean up region value using regex
school_data_transform$region_clean <- str_trim(gsub("((?!US))(\\b[[:alpha:]]{2}\\b)", " ",
                                                    school_data_transform$region, perl = TRUE), side = "right")
...
school_data_transform$school_size_category <- 
  ifelse(school_data_transform$ugds < 1000,"Under 1,000",
...
football_data_transform <- football_data_transform[football_data_transform$Rk != "Rk" 
                                                   & football_data_transform$Notes != "Game Cancelled",] 
...
football_data_transform$school_home <- 
  ifelse(football_data_transform$V1 == "@", "away", 
         ifelse(football_data_transform$Notes == "", "home", "neutral"))
#create field for if the loser was at home, away, or at a neutral site
football_data_transform$opponent_home <- 
  ifelse(football_data_transform$school_home == "away", "home",
...
Ok, now that the dimSchool table is loaded, we have to look at our conformed dimension between the school and the football data. The conformed dimension in the data warehouse between the two data sources is the school name. However, the syntax for school name is different between the two data sources (i.e. Ohio State vs. Ohio State University). Thus, to be able to join these two datasets together, we needed to perform fuzzy text matching.


The different algorithms provided by stringdist:

* Hamming distance: Number of positions with same symbol in both strings. Only defined for strings of equal length. (e.g.) distance(‘abcdd‘,’abbcd‘) = 3
...
* Longest Common Substring distance: Minimum number of symbols that have to be removed in both strings until resulting substrings are identical. (e.g.) distance(‘ABCvDEx‘,’xABCyzDE’) = 5
...
* Soundex determines if the strings are phonetically similar and if so returns a value of 0