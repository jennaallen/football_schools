...
get_football_data <- function() {
  # create urls based on the desired years
...
clean_football_data <- function(original_football_data) {
  ...
  football_data_transform <- original_football_data
...
summary(select_if(school_data_transform, function(col) is.numeric(col) || is.numeric(col)))
...
One tricky aspect of this data set is the conformed dimension. The conformed dimension in the data warehouse between the two data sources is the school name. However, the syntax for school name is different between the two data sources (i.e. Ohio State vs. Ohio State University). Thus, to be able to join the school and football data sets together, we needed to perform fuzzy text matching. We also have to make sure that the school name is unique when we tie the two datasets together. Duplicate school names that represent different schools (e.g. schools of the same name in different locations) in the data can be problematic.
...
dbWriteTable(con, name = "dimSchool", 
             value = school_data_transform[school_data_transform$school_year_start %in% 
...
dimSchoolYear <- create_school_year ()
...
I had to the load the data in two steps to get two rows for each game. First, I loaded the ‘winner’ data. In the ‘winner’ load, all winners were put in the ‘school’ column and the ‘school_win’ column was hardcoded to be ‘1’. 


```{r}
create_factGame <- function() {
  #get SKs
  school_opponent_sks <- dbGetQuery(con, "SELECT school_sk, school_name FROM FootballSchoolDW.dimSchool")
  
  #create unique game id by concantenating the school year with the original game id
  football_data_transform$school_year_sk <- plyr::mapvalues(football_data_transform$game_date, from = dimDate$date_value, to = dimDate$school_year_sk)
  football_data_transform$game_id <- paste0(football_data_transform$school_year_sk, football_data_transform$game_id)
  
  #get school sks
  football_data_transform$school_lookup <- plyr::mapvalues(tolower(football_data_transform$school), from = fuzzy_text_match_final$football_schools, to = fuzzy_text_match_final$schools)
  football_data_transform$school_sk <- plyr::mapvalues(tolower(football_data_transform$school_lookup), from = school_opponent_sks$school_name, to = school_opponent_sks$school_sk)
  
  #get opponent sks
   football_data_transform$opponent_lookup <- plyr::mapvalues(tolower(football_data_transform$opponent), from = fuzzy_text_match_final$football_schools, to = fuzzy_text_match_final$schools)
  ...
  football_data_transform_winner <- football_data_transform[, c("game_id, ")]
  football_data_transform$school_sk <- 
